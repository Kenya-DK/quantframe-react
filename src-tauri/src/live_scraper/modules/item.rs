...
impl<'a> ItemModule<'a> {
    pub async fn check_stock(&self) -> Result<(), AppError> {
        ...
        let price_scraper_interesting_items_names: Vec<String> = match helper::get_column_values(
        ...
        // Get interesting items from stock items if the order mode is sell or both and remove blacklisted items.
        ...
        if order_mode == OrderMode::Buy || order_mode == OrderMode::Sell {
            ...
            let order_ids: Vec<String> = match helper::get_column_values(
                helper::merge_dataframes(vec![
                    current_buy_orders_df.clone(),
                    current_sell_orders_df,
                ])?,
                Some(
                    col("order_type")
                        .eq(lit(order_type))
        ...
        // Get potential items to buy from the price scraper.
        ...
        for item in interesting_items.clone() {
            ...
            if !price_scraper_interesting_items_names.contains(&item) {}
        }
        // logger::info_con(
        //     "ItemModule",
        //     format!("Price Scraper Items: {:?}", price_scraper_interesting_items).as_str(),
        // );
        // logger::info_con(
        //     "ItemModule",
        //     format!("Stock Items: {:?}", stock_items_interesting_items).as_str(),
        // );
