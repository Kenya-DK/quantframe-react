...
impl ItemModule {
    ...
    pub async fn refresh_types(&self) -> Result<Vec<Item>, AppError> {
        ...
        helper::emit_update_initialization_status(
            "Downloading Item Data from Warframe.Market...",
            None,
        );
        ...
        helper::emit_update_initialization_status("Downloading Item Data from Relics.Run...", None);
        ...
        helper::emit_update_initialization_status("Storing Looping through Item Data...", None);
        let mut items: Vec<Item> = Vec::new();
        for item in wfm_items.clone() {
            let relic_data = response.get(&item.id.clone());
            if relic_data.is_some() {
                let mut new = item.clone();
                let set_items = self.get_string_arr_from_json(relic_data.unwrap(), "set_items");
                let tags = self.get_string_arr_from_json(relic_data.unwrap(), "tags");
                let subtypes = self.get_string_arr_from_json(relic_data.unwrap(), "subtypes");
                let mod_max_rank = relic_data.unwrap().get("mod_max_rank").unwrap().as_i64();
                let mut mr_requirement: Option<i64> = Some(0);
                let mut wiki_url: Option<String> = Some("".to_string());
                // Find the item response2 by name property
                let item_details = response2.iter().find(|&x| {
                    x["name"].as_str().unwrap() == item.item_name.replace("(", "").replace(")", "")
                });
                ...
                if item_details.is_some() {
                    ...
                    if item_details["wikiaUrl"].is_string() {
                        wiki_url = item_details["wikiaUrl"].as_str().map(|s| s.to_string());
                    }
                }
    ...
    fn get_string_arr_from_json(&self, json: &Value, key: &str) -> Vec<String> {
