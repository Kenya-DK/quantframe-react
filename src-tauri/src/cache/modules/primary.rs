...
use crate::cache::{client::CacheState, types::CachePrimary};
...
impl PrimaryModule {
    ...
    pub fn get_items(&self) -> Result<Vec<CachePrimary>, Error> {
        let items = self
            .items
            .lock()
    ...
    pub fn load(&self) -> Result<(), Error> {
        ...
        match read_json_file::<Vec<CachePrimary>>(&client.base_path.join(self.path.clone())) {
            Ok(items) => {
                let mut items_lock = self.items.lock().unwrap();
                *items_lock = items;
                info(
                    "Cache:Primary:load",
                    "Loaded Primary items from cache",
