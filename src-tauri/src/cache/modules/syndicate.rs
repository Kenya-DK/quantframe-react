...
impl SyndicateModule {
    ...
    pub fn load(&self, _: &LanguageModule) -> Result<(), Error> {
        match read_json_file_optional::<Vec<CacheSyndicate>>(&self.path) {
            Ok(mut items) => {
                let mut item_lookup = self.syndicate_lookup.lock().unwrap();
                for item in items.iter_mut() {
                    // item.translate(&language);
                    let keys = vec![item.name.clone(), item.unique_name.clone()];
                    item_lookup.insert_value(item.clone(), keys);
                }
                info(
                    "Cache:Syndicate:load",
                    format!("Loaded {} syndicates from cache", item_lookup.len()),
