...
impl AppError {
    ...
    // Custom constructor
    pub fn new_with_level(component: &'static str, eyre_report: eyre::ErrReport, log_level: LogLevel) -> Self {
        AppError { 
            component,
            eyre_report,
            log_level,
    ...
    pub fn extra_data(&self) -> Value {
        let (_before_location, _after_location, json) = get_info(format!("{:?}", self.eyre_report));
        json
    }
...
fn main() {
    let vau = mm();
        match vau {
            ...
            Err(e) => {
                let lev = e.log_level();
                println!("Error while sending message to discord {:?}", lev);
            }
        }
}

fn mm() -> Result<(), AppError> {
    Err(AppError::new_with_level("PriceScraper",LogLevel::Info))
}