...
pub async fn add_riven(riven: web::Json<CreateStockRiven>) -> impl Responder {
    ...
    // Validate the riven
    match cache.riven().validate_create_riven(&mut riven, "--weapon_by name --weapon_lang en --attribute_by upgrades") {
        Ok(_) => (),
        Err(e) => {
    ...
    match StockRivenMutation::create(&app.conn, stock.clone()).await {
        ...
        Err(e) => {
            return HttpResponse::BadRequest().body(serde_json::to_string(&AppError::new(component, eyre::eyre!(e))).unwrap());
        }
    }
