...
export const OrderPanel = ({}: OrderPanelProps) => {
  ...
  useEffect(() => {
    ...
    ordersF = ordersF.sort((a, b) => {
      if (a.order_type == Wfm.OrderType.Buy && b.order_type == Wfm.OrderType.Sell) return 1;
      if (a.order_type == Wfm.OrderType.Sell && b.order_type == Wfm.OrderType.Buy) return -1;
      return 0;
    });
          ...
          <Group gap={5}>
            <ActionWithTooltip
              ...
              loading={
                ...
                refreshOrdersMutation.isPending ||
      ...
      <Group gap={"sm"} mt={"md"}>
        {Object.values(Wfm.OrderType).map((type) => (
          <ColorInfo
            ...
            text={useTranslateOrderType(`${type}`) + `${!statusCount[type] ? "" : ` ${statusCount[type]}`}`}
            tooltip={useTranslateOrderType(`details.${type}`)}
          />
        ))}
      </Group>
                  ...
                  <ActionWithTooltip
                    ...
                    onClick={(e) => {
                      ...
                      modals.openConfirmModal({
                        ...
                        children: <Text size="sm">{useTranslatePrompt("delete.message", { name: order.item?.en?.item_name })}</Text>,
