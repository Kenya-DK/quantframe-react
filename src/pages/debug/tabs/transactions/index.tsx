...
export const TransactionPanel = ({ }: TransactionPanelProps) => {
    ...
    // Translate general
    const useTranslateTabTransactions = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslatePages(`debug.tabs.transaction.${key}`, { ...context }, i18Key)
    const useTranslateDataGridColumns = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateTabTransactions(`datatable.columns.${key}`, { ...context }, i18Key)
    const useTranslateTransactionType = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateEnums(`transaction_type.${key}`, { ...context }, i18Key)
    const useTranslateTransactionItemType = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateEnums(`item_type.${key}`, { ...context }, i18Key)
    const useTranslateErrors = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateTabTransactions(`errors.${key}`, { ...context }, i18Key)
    const useTranslateSuccess = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateTabTransactions(`success.${key}`, { ...context }, i18Key)
    const useTranslatePrompt = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateTabTransactions(`prompts.${key}`, { ...context }, i18Key)
    ...
    useEffect(() => {
        ...
        if (query !== "")
            items = items.filter((item) => item.item_name.toLowerCase().includes(query.toLowerCase()));

        if (filterTransactionType)
            items = items.filter((item) => item.transaction_type === filterTransactionType);

        if (filterItemType)
            items = items.filter((item) => item.item_type === filterItemType);
    ...
    const updateTransactionMutation = useMutation({
        ...
        onSuccess: async (u) => {
            notifications.show({ title: useTranslateSuccess("update_transaction.title"), message: useTranslateSuccess("update_transaction.message", { name: u.item_name }), color: "green.7" });
        },
        onError: (e) => {
            console.error(e);
            notifications.show({ title: useTranslateErrors("update_transaction.title"), message: useTranslateErrors("update_transaction.message"), color: "red.7" })
        }
    })
    const deleteTransactionMutation = useMutation({
        mutationFn: (id: number) => api.transaction.delete(id),
        onSuccess: async () => {
            notifications.show({ title: useTranslateSuccess("delete_transaction.title"), message: useTranslateSuccess("delete_transaction.message"), color: "green.7" });
        },
        onError: (e) => {
            console.error(e);
            notifications.show({ title: useTranslateErrors("delete_transaction.title"), message: useTranslateErrors("delete_transaction.message"), color: "red.7" })
        }
    })
    // Modal's
    const OpenUpdateModal = (transaction: UpdateTransactionDto) => {
        modals.open({
            ...
            children: (<UpdateTransaction value={transaction} onSubmit={async (data) => {
            ...
            <Group gap={"md"} mt={"md"} grow>
                <Group>
                    {Object.values([TransactionType.Purchase, TransactionType.Sale]).map((status) => (
                        <ColorInfo active={status == filterTransactionType} key={status} onClick={() => {
                            setFilterTransactionType(s => s === status ? undefined : status);
                        }} infoProps={{
                            "data-color-mode": "bg",
                            "data-trade-type": status,
                        }} text={useTranslateTransactionType(`${status}`) + `${statusCount[status] == 0 ? "" : ` (${statusCount[status]})`}`} tooltip={useTranslateTransactionType(`details.${status}`)} />
                    ))}

                </Group>
                <Group justify="flex-end" >
                    {Object.values(TransactionItemType).map((type) => (
                        <ColorInfo active={type == filterItemType} key={type} onClick={() => {
                            setFilterItemType(s => s === type ? undefined : type);
                        }} infoProps={{
                            "data-color-mode": "bg",
                            "data-item-type": type,
                        }} text={useTranslateTransactionItemType(`${type}`) + `${statusCount[type] == 0 ? "" : ` (${statusCount[type]})`}`} tooltip={useTranslateTransactionItemType(`details.${type}`)} />
                    ))}
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        render: ({ item_name, sub_type, properties }) => (
                            <TextTranslate color="gray.4" i18nKey={useTranslateDataGridColumns("name.value", undefined, true)} values={{
                                name: item_name,
                    ...
                    },
                    {
                        ...
                        sortable: true,
                        render: ({ item_type }) => (
                            <Text data-color-mode="text" data-item-type={item_type}>{useTranslateTransactionItemType(item_type)}</Text>
                        ),
                    },
                    {
                        accessor: 'quantity',
                                ...
                                <ActionWithTooltip
                                    ...
                                    onClick={(e) => {
                                        ...
                                        modals.openConfirmModal({
                                            ...
                                            children: (
                                                <Text size="sm">
                                                    {useTranslatePrompt('delete.message', { name: row.item_name })}
                                                </Text>
                                            ),
