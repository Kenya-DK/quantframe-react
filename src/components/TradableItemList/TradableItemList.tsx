...
export function TradableItemList({ onAddItem, onAddAll, availableItems }: TradableItemListProps) {
  ...
  const useTranslate = (key: string, context?: { [key: string]: any }, i18Key?: boolean) =>
    useTranslateComponent(`tradableItem_list.${key}`, { ...context }, i18Key);
  ...
  const useTranslateSearchButtons = (key: string, context?: { [key: string]: any }, i18Key?: boolean) =>
  ...
  const GetFilteredItems = () => {
    ...
    itemFilter = SortItems(itemFilter, {
    ...
    <Box>
            ...
            <Paper radius="md" p={"sm"} mt={"md"}>
              <Grid>
                ...
                <Grid.Col span={6}>
                  <Box>
                    <Text size="sm" mt="xl">
                      {useTranslateFormFields("trade_tax.label", {
                        min: filterForm.values.tradeTaxRange[0],
                        max: filterForm.values.tradeTaxRange[1],
                      })}
                    </Text>
                    <RangeSlider
                      w={300}
                  ...
                  <Box>
                    <Text size="sm" mt="xl">
                      {useTranslateFormFields("mr_requirement.label", {
                        min: filterForm.values.mrRequirementRange[0],
                        max: filterForm.values.mrRequirementRange[1],
                      })}
                    </Text>
                    <RangeSlider
                      w={300}
      ...
      <Box mt={"md"} className={classes.dataTable} data-filter={is_filter_open}>
        <DataTable
          ...
          onRowClick={(row) => {
            if (onAddItem) onAddItem(row.record);
          }}
          // define columns
          columns={[
            {
