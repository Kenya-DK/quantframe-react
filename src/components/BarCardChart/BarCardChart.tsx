...
export function BarCardChart({ labels, title, context, chartStyle, tooltipShowColor, boxWidth, boxHeight, tooltipCallback, showDatasetLabels, datasets }: BarCardChartProps) {
	...
	return (
		<Paper className={classes.root} >
			<Box className={classes.chartContainer}>
				{useMemo(
					() => (
						<Bar style={{ ...chartStyle }} className={classes.chartCanvas} options={GetDefaultOptions(theme.colors.gray[0], showDatasetLabels, tooltipShowColor, boxWidth, boxHeight, tooltipCallback) as any} data={cData} />
					),
					[datasets]
				)}
			</Box>
