...
export function OrderDetails({ value }: OrderDetailsProps) {
  ...
  // Translate general
  const useTranslateStockRivenInfo = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateComponent(`order_details.${key}`, { ...context }, i18Key)
  ...
  const useTranslateButtons = (key: string, context?: { [key: string]: any }, i18Key?: boolean) => useTranslateStockRivenInfo(`buttons.${key}`, { ...context }, i18Key)
    ...
    <Tabs defaultValue="general" h={"75vh"}>
      ...
      <Tabs.Panel value="general">
        <Grid>
          <Grid.Col span={6}>
            ...
            <Group mt={"md"} grow>
              <Button color="blue" variant="outline" onClick={() => { window.open(`https://warframe.market/items/${value.item?.url_name}`, '_blank') }}>{useTranslateButtons("wfm")}</Button>
              {item && <Button color="blue" variant="outline" onClick={() => { window.open(item.wiki_url, '_blank') }}>{useTranslateButtons("wiki")}</Button>}
            </Group>
          </Grid.Col>
          <Grid.Col span={6}>
            ...
            {
              (value.info?.price_history?.length || 0) > 0 && (
                value.info?.price_history.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 5).map((price, index) => (
      ...
      <Tabs.Panel value="orders">
        <ScrollArea h={"70.5vh"}>
          ...
          {(value.info?.orders?.length || 0) > 0 && value.info?.orders?.slice(0, 5).map((auction, index) => <OrderListItem key={index} order={auction} />)}
        </ScrollArea>
      </Tabs.Panel>
    </Tabs>
