...
export function OverviewTab({ value }: OverviewTabProps) {
  ...
  const useTranslateTab = (key: string, context?: { [key: string]: any }, i18Key?: boolean) =>
    useTranslateModals(`wish_list_details.tabs.overview.${key}`, { ...context }, i18Key);
      ...
      <Grid.Col span={6}>
        ...
        <Group grow>
          <TextInput
            ...
            readOnly
          />
          <TextInput label={useTranslateFields("maximum_price")} value={value.stock.maximum_price || "N/A"} readOnly />
          <TextInput label={useTranslateFields("quantity")} value={value.stock.quantity || "N/A"} readOnly />
        </Group>
        <Group grow>
