...
export type RivenFilterProps = {
	...
	onSubmit: (values: StockRivenFilter) => void;
}
		...
		<Box w={"100%"}>
			<form onSubmit={form.onSubmit((data) => {
				...
				<Collapse in={form.values.enabled}>
					<Grid>
						<Grid.Col span={6}>
							<MinMax label={useTranslateFormFields('re_rolls.label')} value={form.values.re_rolls} onChange={(re_rolls) => {
								form.setFieldValue('re_rolls', re_rolls);
							}} />
							<MinMax label={useTranslateFormFields('rank.label')} value={form.values.rank} onChange={(rank) => {
								form.setFieldValue('rank', rank);
							}} />
							<MinMax label={useTranslateFormFields('mastery_rank.label')} value={form.values.mastery_rank} onChange={(mastery_rank) => {
						...
						<Grid.Col span={6}>
							...
							<NumberInput
								...
								onChange={(event) => {
									...
									if (value == 0)
										form.setFieldValue('similarity', undefined);
									else
										form.setFieldValue('similarity', value);
								}}
								suffix=" %"
								mt="md"
							/>
							<Stack mt="md" gap="md">
								{(form.values.attributes ?? []).map((attribute, index) => (
									<RivenFilterAttribute key={index} value={attribute} onChange={(value) => {
