import { ActionIcon, Divider, Flex, Grid, Group, Image, Paper, Text, Tooltip } from "@mantine/core";
import { StockRivenDto, Wfm } from "$types/index";
...
import { getOrderStatusColorCode } from "@utils/index";
interface AuctionProps {
  auction: Wfm.Auction<string>;
  stock_item?: StockRivenDto;
  onImport?: (auction: Wfm.Auction<string>) => void;
  onUpdate?: (auction: Wfm.Auction<string>) => void;
}


export default function Auction({ stock_item, auction, onImport }: AuctionProps) {
  ...
  const GetColor = () => {
    if (stock_item && !auction.visible) {
      return Wfm.OrderStatus.Inactive;
    } else if (stock_item && auction.visible) {
      return Wfm.OrderStatus.Live;
    } else {
      return Wfm.OrderStatus.Pending;
    }
  }
  return (
    <Paper sx={{
      ...
      borderLeftColor: getOrderStatusColorCode(GetColor()),
      padding: "10px",
    }}>
      <Group grow position="apart" spacing="xs">
        ...
        <Group position="right" spacing={"sm"}>
          {(!stock_item && auction.is_direct_sell) && (
            <>
      ...
      <Group grow mt={12}>
        ...
        <Group position="right" spacing={"sm"}>
          {(stock_item) && (
            <>
              <TextColor i18nKey="components.auction.bought" values={{ bought: stock_item?.price || 0 }} />
            </>
          )}
        </Group>
      </Group>
